...
# %% auto #0
__all__ = ['BLOCKED_HOSTS', 'http_get', 'http_post', 'fossick_cache', 'syncy', 'html_title', 'html2md', 'html2text_fast', 'to_md',
           'to_text', 'BlockedURL', 'check_url', 'BrowserUnavailable', 'http_page', 'get_page', 'fetch',
           'browser_session', 'crawl', 'get_options', 'fetch_all', 'get_pdf', 'read_arxiv', 'lookup_doi', 'clean_md',
           'orphan_vals', 'scrambled_layout', 'fix_layout', 'ocr_parse', 'pdf2md', 'url2nb', 'pdf2nb', 'gh_clone',
...
import ast, re, shutil, time, os, sys, base64, hashlib, ipaddress, socket
...
#: Readability returns nothing at all on a page with no article in it — a Hacker News front page
#: converts to 0 characters, and `search()` drops the source as unreadable. Below this, take the
#: whole document instead. Kept small on purpose: a short summary readability *chose* is its call
#: to make, and `readability=False` is how a caller overrides that.
_MIN_ARTICLE = 32
...
def html2md(s:str, ignore_links=True, ignore_images=False,
    ...
    if not readability: return o.handle(_clean_html(s))
    md = o.handle(Document(s).summary(keep_all_images=True))
    return md if len(md.strip()) >= _MIN_ARTICLE else o.handle(_clean_html(s))

def html2text_fast(s:str,             # raw HTML
                   main:bool=True,    # keep the main content only, dropping nav and furniture
                   links:bool=False,  # keep link targets alongside their text
                  ) -> str:
    '''Plain text from HTML through resiliparse, which extracts and converts in one C++ pass.

    Roughly 7x `to_md` on an article page, and it reads pages readability returns nothing for. It
    returns text, not markdown, so it is for callers that were going to strip the markup anyway.'''
    from resiliparse.extract.html2text import extract_plain_text
    if not s.strip(): return ''
    return extract_plain_text(s, main_content=main, links=links)
...
def to_md(res_or_html,  # Page dict (from fetch/crawl) or raw HTML string
    ...
    return _sel_convert(html, lambda h: html2md(str(h), ignore_links, ignore_images, readability),
                        sel, multi, wrap_tag)

def _sel_convert(html, conv, sel=None, multi=False, wrap_tag=None):
    'Apply `conv` to `html`, or to each CSS-selected fragment of it.'
    if sel:
	    outs = L(Selector(html).css(sel)).map(lambda m: conv(m.get()))
	    if multi: return '\n'.join(outs.map(lambda m: _wrap_md(m, wrap_tag) if wrap_tag else m))
	    html = str(outs[0]) if outs else ''
    return conv(html)

def to_text(res_or_html,   # Page dict (from fetch/crawl) or raw HTML string
            sel:str=None,  # CSS selector to extract before conversion
            multi:bool=False,  # Return all selector matches joined
            wrap_tag:str=None,  # Wrap each multi-result in <wrap_tag>...</wrap_tag>; only used when multi=True
            main:bool=True,  # Keep the main content only, dropping nav and furniture
            links:bool=False,  # Keep link targets alongside their text
            ) -> str:
    'Plain text from a Page dict or HTML string. The cheap path when the caller does not need markdown.'
    return _sel_convert(_html(res_or_html), lambda h: html2text_fast(str(h), main, links),
                        sel, multi, wrap_tag)
...
def get_page(url, method='GET', payload=None, heavy=False, stealthy=False, session=False,
             allow_private=None, **kw) -> Response:
