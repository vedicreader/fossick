...
# %% auto #0
__all__ = ['http_get', 'http_post', 'save_path', 'arun', 'html2md', 'to_md', 'fetch', 'crawl', 'get_options', 'fetch_all',
           'get_pdf', 'read_arxiv', 'lookup_doi', 'url2nb', 'pdf2nb', 'read_gh_repo', 'read_gh_file', 'compile_pattern',
           'find_xhr', 'paginate_api', 'download_files', 'search_yt', 'read_yt', 'download_yt', 'repo_root',
           'mv_skill_md']
...
def download_files(url:str,
    ...
    rx = compile_pattern(pattern)
    dl_kw = {k: v for k, v in kw.items() if k == 'verify'}
    ...
    saved, seen = L(), set()
    for pg in pages:
        for a in Selector(pg.html).css(file_sel):
            ...
            r = http_get(abs_url, **dl_kw)
            if r.status_code == 200: fname.write_bytes(r.content); saved.append(fname)
    return saved
