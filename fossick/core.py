...
# %% auto #0
__all__ = ['http_get', 'http_post', 'save_path', 'arun', 'clean_md', 'html2md', 'extract_article', 'to_md', 'fetch', 'url2md',
           'crawl', 'get_options', 'fetch_all', 'get_pdf', 'read_arxiv', 'lookup_doi', 'url2nb', 'pdf2nb',
           'read_gh_repo', 'read_gh_file', 'compile_pattern', 'find_xhr', 'paginate_api', 'search_yt', 'read_yt',
...
def extract_article(html:str) -> str:
    "Strip nav/footer/aside noise and return the main article HTML using lxml"
    from lxml.html import fromstring, tostring
    root = fromstring(html)
    for tag in ('script','style','noscript','iframe','nav','footer','aside','button','form','input','select','textarea'):
        for el in root.findall(f'.//{tag}'):
            p = el.getparent()
            if p is not None: p.remove(el)
    for sel in ('article', 'main'):
        els = root.findall(f'.//{sel}')
        if els:
            best = max(els, key=lambda e: len(e.text_content()))
            if len(best.text_content()) > 100:
                return tostring(best, encoding='unicode', method='html')
...
def to_md(page_or_html, # Page dict (from fetch/crawl) or raw HTML string
          ...
          extract:bool=False, # Strip nav/banner noise via lxml before converting
         ) -> str:
    "Convert a Page dict or HTML string to clean markdown"
    html = _html(page_or_html)
    if extract: html = extract_article(html)
...
# %% ../nbs/00_core.ipynb #0afcdef2
def _build_reader_js() -> str:
    "Combine turndown + reader scripts into a single evaluatable IIFE"
    _d = Path(__file__).parent / 'js'
    parts = [(_d/'turndown.js').read_text(), (_d/'turndown-plugin-gfm.js').read_text(), (_d/'reader.js').read_text()]
    return '(function(){\n' + '\n'.join(parts) + '\n})()'

async def _async_url2md(url: str) -> str:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await browser.close()
    return result.get('markdown', '') if isinstance(result, dict) else str(result or '')

def url2md(url:str,         # URL to fetch and convert
           heavy:bool=True, # Use Playwright + turndown.js (True) or lxml light path (False)
           **kw,            # Extra kwargs passed to fetch() when heavy=False
           ) -> str:
    "Fetch `url` and return clean article markdown, stripping nav/banner/sidebar noise"
    if not heavy: return to_md(fetch(url, **kw), extract=True)
    import asyncio
    try: asyncio.get_running_loop()
    except RuntimeError: return asyncio.run(_async_url2md(url))
    import concurrent.futures
    with concurrent.futures.ThreadPoolExecutor() as pool: return pool.submit(asyncio.run, _async_url2md(url)).result()
...
@fdelegates(fetch)
def url2nb(url, nb_path=None, extract=True, **kwargs):
	...
	md = url2md(url) if extract else to_md(fetch(url, **kwargs))
