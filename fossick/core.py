...
# %% auto #0
__all__ = ['BLOCKED_HOSTS', 'OBSCURA_PORT', 'http_get', 'http_post', 'fossick_cache', 'syncy', 'html_title', 'html2md', 'to_md',
           'BlockedURL', 'check_url', 'obscura_bin', 'obscura_ws', 'BrowserUnavailable', 'http_page', 'get_page',
           'fetch', 'browser_session', 'crawl', 'get_options', 'fetch_all', 'get_pdf', 'read_arxiv', 'lookup_doi',
           'pdf2md', 'url2nb', 'pdf2nb', 'gh_clone', 'read_gh_repo', 'read_gh_file', 'compile_pattern', 'find_xhr',
           'replay_xhr', 'json_records', 'paginate_api', 'download_files', 'search_yt', 'read_yt', 'download_yt',
           'what_is', 'md_title', 'pdf_pages', 'read', 'repo_root', 'mv_skill_md', 'clean_md', 'fix_layout',
           'needs_ocr', 'ocr_parse', 'orphan_vals', 'pdf_doc', 'pdf_md']
...
import ast, re, shutil, time, os, sys, base64, hashlib, ipaddress, socket, subprocess
...
#: Tags whose contents are code or markup, never prose
_DROP_TAGS = ('script','style','noscript','template')
#: Adjacent inline tags run their words together ('trace IDs'+'tool call logs'), so they get a space
_INLINE_TAGS = {'span','a','b','i','em','strong','code','small','label','td','th'}

def _clean_html(s:str) -> str:
    'Drop script/style/noscript/template and comments from `s`, and space out adjacent inline tags'
    try: doc = lxml.html.fromstring(s)
    except Exception: return s
    for el in doc.xpath('|'.join(f'//{t}' for t in _DROP_TAGS)+'|//comment()'): el.drop_tree()
    for el in doc.iter():
        if el.tag in _INLINE_TAGS and not el.tail:
            nxt = el.getnext()
            if nxt is not None and nxt.tag in _INLINE_TAGS and (nxt.text or ' ')[:1] not in ',.;:!?)]}%': el.tail = ' '
...
def html2md(s:str, ignore_links=True, ignore_images=False,
    ...
    return o.handle(Document(s).summary(keep_all_images=True) if readability else _clean_html(s))
...
def to_md(res_or_html,  # Page dict (from fetch/crawl) or raw HTML string
    ...
    _md = lambda h: html2md(str(h), ignore_links, ignore_images, readability)
    if sel:
...
# %% ../nbs/00_core.ipynb #d7fe1f81
#: Hostnames that name an internal service whatever they resolve to. Checked before DNS, since a
#: cloud metadata endpoint is worth refusing even where it does not resolve.
...
def _env_allows_private() -> bool:
    return (os.environ.get('FOSSICK_ALLOW_PRIVATE') or '').strip().strip('\'"').lower() in (
        '1', 'true', 'yes', 'on')

def _bad_ip(addr:str) -> bool:
    "Is `addr` an address no outbound fetch should reach?"
...
def check_url(url:str,                   # the URL about to be fetched
    ...
    if u.scheme and u.scheme.lower() not in ('http', 'https', ''):
        raise BlockedURL(f'fetch is http/https only, got {u.scheme!r}: {url}')
    try: h = (u.hostname or '').strip().rstrip('.').lower()
    except ValueError: h = ''
    if not h: raise BlockedURL(f'no hostname to fetch: {url}')
    hint = ' — pass allow_private=True or set FOSSICK_ALLOW_PRIVATE=1 if that is intended'
    if h in BLOCKED_HOSTS: raise BlockedURL(f'{h} is an internal host{hint}')
    if _bad_ip(h): raise BlockedURL(f'{h} is a private or reserved address{hint}')
    try: infos = socket.getaddrinfo(h, u.port or (443 if u.scheme == 'https' else 80),
                                    proto=socket.IPPROTO_TCP)
    except socket.gaierror as e:
        raise BlockedURL(f'cannot resolve {h}: {e}') from e
...
# %% ../nbs/00_core.ipynb #a1b2c3d4e5f60002
#: Where `engine='obscura'` looks for its CDP server. Deliberately not 9223: that port belongs to the
#: debug Chrome, and the two are different browsers with different cookie jars.
OBSCURA_PORT = int(os.getenv('FOSSICK_OBSCURA_PORT', 9224))

def _cdp_ready(port, host='127.0.0.1') -> bool:
    "True once a CDP endpoint on `port` serves `/json/version` — Chrome, obscura, anything speaking the protocol."
    import httpx
    try: return bool(httpx.get(f'http://{host}:{port}/json/version', timeout=.5).json().get('webSocketDebuggerUrl'))
    except Exception: return False

def obscura_bin() -> str:
    "The `obscura` binary: `$FOSSICK_OBSCURA`, else the first one on `PATH`."
    b = os.getenv('FOSSICK_OBSCURA') or shutil.which('obscura')
    if not b: raise BrowserUnavailable(
        "no obscura binary found — engine='obscura' needs one on PATH or named by $FOSSICK_OBSCURA. "
        'Releases: https://github.com/h4ckf0r0day/obscura/releases')
    return b

def obscura_ws(port=None, storage_dir=None, stealth=None, timeout=15) -> str:
    """Websocket debugger URL of an `obscura serve` on `port`, starting one if needed. For scrapling's `cdp_url=`.

    obscura is a Chromium-free CDP server. Measured on this machine: ~10ms to start and ~25MB
    resident, against ~200ms and ~410MB for a headless Chrome. Warm, per page, the two are within a
    few ms of each other — the win is the startup and the footprint, not the render. So this pays off
    for many short-lived JS fetches and for crawling wide, and buys nothing for one long session."""
    port = port or OBSCURA_PORT
    # Stealth is on unless $FOSSICK_OBSCURA_STEALTH turns it off: its TLS impersonation is most of the
    # reason to scrape with obscura, but a MITM proxy (corporate egress, mitmproxy) resets the
    # impersonated handshake, and every fetch fails on Connect until you drop it.
    if stealth is None: stealth = (os.getenv('FOSSICK_OBSCURA_STEALTH') or '1').strip().lower() not in ('0', 'false', 'no', 'off')
    if not _cdp_ready(port):
        d = Path(storage_dir).expanduser() if storage_dir else fossick_cache('obscura')
        d.mkdir(parents=True, exist_ok=True)
        args = [obscura_bin(), 'serve', '--port', str(port), '--storage-dir', str(d)]
        if stealth: args.append('--stealth')
        # obscura refuses private targets on its own; `check_url` already gates that on our side, so
        # keep the two agreeing rather than having the browser veto a fetch fossick allowed.
        if _env_allows_private(): args.append('--allow-private-network')
        subprocess.Popen(args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
        deadline = time.time() + timeout
        while not _cdp_ready(port):
            if time.time() > deadline: raise TimeoutError(f'obscura serve on :{port} not ready after {timeout}s')
            time.sleep(.05)
    import httpx
...
@cache
def _browser_fetchers():
    """scrapling's browser fetchers, imported on demand.

    Importing them can fail outright — scrapling generates a browser fingerprint at import time and
    browserforge raises when it has no header set for the running platform. That surfaces as a bare
    `ValueError` from a `from ... import ...` line, which every caller here used to treat as "no
    results". Re-raise it as something that names the actual problem."""
    try:
        from scrapling.fetchers import StealthyFetcher, DynamicFetcher
        return StealthyFetcher, DynamicFetcher
    except Exception as e:
        raise BrowserUnavailable(
            f'scrapling browser fetchers unavailable ({type(e).__name__}: {e}). '
            'heavy/stealthy fetching, fetch(auto=True) escalation and google() all need them — '
            'try `scrapling install`, or upgrade scrapling/browserforge.') from e
...
def get_page(url, method='GET', payload=None, heavy=False, stealthy=False, session=False,
             engine=None, allow_private=None, **kw) -> Response:
	"""Fetch `url`. `session=True` (or a port int) drives the persistent debug Chrome via CDP, reusing its logged-in cookies.

	`engine='obscura'` runs the JS tier on a Chromium-free CDP server instead of a headless Chrome.

	Every fetch in fossick arrives here, which is why the private-address check lives here and not in
	`fetch`: `crawl`, `replay_xhr`, `download_files` and `http_get` would each have to remember."""
	assert method.upper() in ('GET', 'POST'), 'Only GET and POST methods are supported'
	# Reject an impossible combination before starting a browser or resolving a name for it.
	if engine not in (None, 'obscura'): raise ValueError(f'unknown engine {engine!r}; use None (Chrome) or "obscura"')
	if engine and session: raise ValueError(
		'engine="obscura" and session=True name different browsers: session reuses the logged-in debug '
		'Chrome, obscura keeps its own cookie jar')
	if engine and stealthy: raise ValueError(
		'engine="obscura" cannot back stealthy=True: that tier is Camoufox, a real Firefox')
	check_url(url, allow_private)
	if engine == 'obscura':
		kw.setdefault('cdp_url', obscura_ws())
		heavy = True
	if session:
	...
	StealthyFetcher, DynamicFetcher = _browser_fetchers()
...
def _warn_once(key, msg):
    "Print `msg` to stderr the first time `key` fails — silent degradation is how bad data gets in."
...
def _fetch_auto(url, sel=None, tiers=_AUTO_TIERS, **kw) -> Response:
    ...
    Every tier's failure is recorded on `.errs`. When the browser tiers can't even load, escalation
    silently degrades to "plain HTTP only" and the caller gets the bot wall back as if it were the
    page — so say it once, loudly, rather than returning a challenge page that looks like content."""
    page, errs = None, []
    for opts in tiers:
        tier = '+'.join(k for k, v in opts.items() if v) or 'plain'
...
def fetch(url:str,                # URL to fetch
          ...
          engine:str=None,        # Browser for the JS tier: None launches Chrome, 'obscura' uses a Chromium-free CDP server
    ...
    # `auto` picks the browser per tier (its last two tiers are Camoufox and the debug Chrome), so an
    # `engine` for all of them would be a promise this cannot keep. Say so instead of half-honouring it.
    if auto and engine: raise ValueError('engine= applies to one fetch; auto= escalates through tiers that pick their own browser')
    if auto: return _fetch_auto(url, sel=sel, method=method, payload=payload, capture_xhr=capture_xhr,
                            allow_private=allow_private, **kw)
    heavy = heavy or capture_xhr
    if capture_xhr: kw.setdefault('capture_xhr', '.*')
    page = get_page(url, method=method, payload=payload, heavy=heavy, stealthy=stealthy, session=session,
                    engine=engine, allow_private=allow_private, **kw)
...
def gh_clone(path_or_url:str  # GitHub URL, SSH address, or a local path (returned as-is)
             ) -> Path:
    'The local clone for a GitHub repo, cloned or refreshed on demand. A local path passes through.'
    return _get_git_repo(rs) if (rs := _gh_remotes(path_or_url)) else Path(path_or_url)
...
def json_records(data,                # any decoded JSON payload
    ...
    """The longest list of dicts anywhere in `data` — how an API's records are found when no key is named.

    A JSON API puts its rows where it likes: at the top level, under `results`, or under
    `data.products.items`. Scanning only the top level finds the first two and silently misses the
    third, which reads as an empty response rather than as a payload nobody looked into."""
    best, at = [], None
    def walk(o, d=0, k=None):
        nonlocal best, at
        if d > max_depth: return
        if isinstance(o, list):
            ds = [x for x in o if isinstance(x, dict)]
            if len(ds) >= min_len and len(ds) > len(best) and (key is None or k == key): best, at = ds, k
            for x in o[:20]: walk(x, d+1, k)
        elif isinstance(o, dict):
            for kk, v in o.items(): walk(v, d+1, kk)
    walk(data)
    return (best, at) if with_key else best
...
def paginate_api(url:str,                      # API endpoint URL
    ...
    for page_num in range(page_start, max_pages + page_start):
        ...
        elif results_field and (raw := data.get(results_field)) is not None:
            items = list(raw.values()) if isinstance(raw, dict) else raw
        else:
            # a named field that is not at the top level is still a named field: look for it, and
            # for records generally, anywhere in the payload before falling back to the shallow rule
            items = json_records(data, min_len=1, key=results_field)
            if not items:
