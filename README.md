...
attaches to a running Chrome browser: `pg.ax_tree()` reads the page
state and `pg.fill_text()` / `pg.click_and_wait()` act on it.
...
Use `pg.ax_tree()` to read the accessibility tree, then pass node IDs
from the output to `pg.fill_text()` and `pg.click_and_wait()`.
[`syncy()`](https://vedicreader.github.io/fossick/cdp.html#syncy) runs
any async CDP call synchronously.

Always print the full `ax_tree()` output and read node IDs from it —
never truncate. IDs change on every navigation, so re-read the tree
after each action. `fill_text()` and `click_and_wait()` handle all waits
internally — no `time.sleep()` needed.
...
[`setup_chrome_daemon()`](https://vedicreader.github.io/fossick/cdp.html#setup_chrome_daemon)
installs a system service that starts Chrome with remote debugging at
login. Run once per machine — macOS (launchd), Linux (systemd), or
