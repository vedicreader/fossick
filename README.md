...
Two things decide whether a result is *usable* before relevance gets a say. **Where**: `region` defaults to `'auto'`, which reads the country off the query itself, so "rebuild my garage in australia" is searched from `au-en` rather than answering with US building rules. The two halves of a region are resolved separately — where to search is a fact about the subject, what language to answer in is a fact about the asker — so an English question about France gets `fr-en`, and a query naming two countries abstains rather than picking one. **When**: `timelimit='d'|'w'|'m'|'y'` restricts to the last day, week, month or year, on `search()`, `google()` and `research()` alike.

`research()` asks the question more than once when the question contains more than one. *"rebuild my garage in australia, cheap options, what is the regulation on it, material options"* is three questions, and `plan()` splits it into the three searches it is really asking. The per-facet result lists are **interleaved rather than fused**, which is the whole point: RRF is right for combining backends answering one question and wrong for combining facets of different ones, because it rewards the page that ranks for all of them — on a three-part question, exactly the "complete guide" written to rank for all of them. The original query is always the first list, and `plan()` abstains on anything without a second question, so simple queries cost one fan-out as before.

`research()` also curates before it fetches: known content mirrors are dropped, sources are ranked by authority *for the kind of question being asked* (a `.gov.au` page for a regulation, a changelog for a release, a forum thread for lived experience), and no single publisher is allowed to own the page. `res['curation']` reports what that did, including a diversity score — five urls carrying one story are five urls and one source. Pass `curated=False` to turn it off.
