...
[`read`](https://vedicreader.github.io/fossick/core.html#read) puts one call in front of every reader. [`what_is`](https://vedicreader.github.io/fossick/core.html#what_is) decides which one a target names; the
result comes back as `(ok, kind, title, source, text, meta)` whatever it was.

``` python
for t in ['https://arxiv.org/abs/1706.03762', 'https://youtu.be/dQw4w9WgXcQ',
          'https://example.com/post', 'README.md']:
    r = read(t)
    print(f'{r.kind:8s} {r.ok}  {r.title[:40]}')
```

`dir` and `github` name a tree rather than a document, so they come back with the local path in
`meta` and no text. Everything else carries markdown in `text`; `read(url, pages=True)` gives a
PDF as `[(page_no, text)]` so a citation can name a page.

When a target cannot be read, a bot wall, a video with no transcript, a URL that is not a PDF, `ok` is False and `skipped` says why, rather than an exception or an empty string.
